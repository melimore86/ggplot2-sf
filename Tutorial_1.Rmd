...
package `maps` provides maps of the USA, with state and county
...
# Print Feature 
<!-- It's actually the viewport feature, not so much the print -->
<!-- feature. Every ggplot plot is actually 'print'ed. -->
...
<!-- I created a data frame with the lines connecting the locations -->
<!-- and the insert. In the spirit of sf, it would be great to -->
<!-- actually have these as spatial lines, but I was lazy. See if you -->
<!-- can do it. -->

```{r printmap_mapsdata, include=TRUE}
map_line1 <- data.frame(lon = c(-80.15, -85), lat = c(26.75, 28))
map_line2 <- data.frame(lon = c(-80.15, -85), lat = c(26.25, 25))

full <- ggplot(data = world) +
    geom_sf(fill = "cornsilk") +
    geom_sf(data = locations, size = 3, shape = 23, fill = "darkred") +
    coord_sf(xlim = c(-87.35, -79.50), ylim = c(24.10, 30.80)) +
    geom_line(data = map_line1, aes(x =  lon, y = lat)) +
    geom_line(data = map_line2, aes(x =  lon, y = lat)) +
    xlab("Longitude")+ ylab("Latitude") +
    ggtitle ("Full map with inserts") +
    theme(panel.grid.major = element_line(color = gray(.5),
        linetype = "dashed", size = 0.5),
        panel.background = element_rect(fill = "lightskyblue1"))

locations <- data.frame(name = c("A", "B"), lon = c(-80.15, -80.15), lat = c(26.75, 26.25))
locations <- st_as_sf(locations, coords = c("lon", "lat"), crs = 4326, agr = "constant")

library("ggthemes")

zoom1 <- ggplot(data = world) +
    geom_sf(fill = "cornsilk") +
    geom_sf(data = locations, size = 3, shape = 23, fill = "darkred") +
    coord_sf(xlim = c(-80.3, -80), ylim = c(26.6, 26.9), expand = FALSE) +
    ggtitle ("Location A") + 
    xlab("Longitude") + ylab("Latitude") +
    theme_map() +
    theme(panel.grid.major = element_line(color = gray(.5),
...
zoom2 <- ggplot(data = world) +
    geom_sf(fill = "cornsilk") +
    geom_sf(data = locations, size = 3, shape = 23, fill = "darkred") +
    coord_sf(xlim = c(-80.3, -80), ylim = c(26.1, 26.4), expand = FALSE) +
    ggtitle ("Location B") + 
    xlab("Longitude") + ylab("Latitude") +
    theme_map() +
    theme(panel.grid.major = element_line(color = gray(.5),
...
```{r printmap_vp}
vp1 <- viewport(width = 0.37, height = 0.32, x = 0.15, y = 0.41, 
    just = c("left", "bottom"))
vp2 <- viewport(width = 0.37, height = 0.32, x = 0.15, y = 0.09, 
    just = c("left", "bottom"))
```
...
png("Florida_insert.png", width = 7, height = 7, units = "in", res = 300)
full
print(zoom1, vp = vp1)
print(zoom2, vp = vp2)
dev.off()
...
list.plots <- list(full, zoom1, zoom2)

align_three_plots <- function(list.plots, family = "") {
    
    gg <- ggplot() + 
        coord_equal(xlim = c(0, 80), ylim = c(0, 51), expand = TRUE) + 
        annotation_custom(ggplotGrob(rmarkplot), xmin = 0, xmax = 57, 
            ymin = 0, ymax = 50) +
        annotation_custom(ggplotGrob(rmarkplot2), xmin = 58, xmax = 80, ymin = 25, ymax = 51) +
        annotation_custom(ggplotGrob(rmarkplot3), xmin = 58, xmax = 80, ymin = 0, ymax = 29) +
        labs(x = NULL, y = NULL) +
        theme_void()
    
    df.arrows <- data.frame(id = 1:2, x = c(45.6, 45.4), y = c(21.7, 
        19.8), xend = c(58, 58), yend = c(35, 12))
    
    gg <- gg +
        geom_segment(data = df.arrows %>% filter(id == 
        1), aes(x = x, y = y, xend = xend, yend = yend), arrow = arrow(type = "closed", 
        length = unit(0.25, "cm"))) +
        geom_segment(data = df.arrows %>% filter(id == 2), aes(x = x, y = y, xend = xend, yend = yend), arrow = arrow(type = "closed", length = unit(0.25, "cm")))
}
