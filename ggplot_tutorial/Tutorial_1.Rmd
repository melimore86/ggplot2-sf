...
This tutorial is the first part of a series divided in three steps:
...
We can check that the map was properly retrieved and converted into an `sf` object, and plot it with `ggplot2`:
...
## Scale bar and North arrow (`legendMap::scalebar`)

Several packages are available to create a scale bar on a map (e.g. `prettymapr`, `vcd`, or `ggsn`). We introduce here the package [`legendMap`](https://github.com/3wen/legendMap), which provides an easy-to-use function `scale_bar` that allows to add simultaneously the north symbol and a scale bar into the `ggplot` map:
...
* Grid lines on the map: Using `panel.grid.major` and `panel.grid.minor`, grid lines can adjusted. Here we set them to a gray color and dashed lines to clearly distinguish country borders from grid lines;
...
ggsave("Floridamap_web.png", width = 6, height = 6, dpi = "screen")
```
