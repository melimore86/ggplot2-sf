...
vp_topleft <- viewport(width = 0.25, height = 0.5, x = 0.11, y = 0.97, just = c("left", "top"))
vp_bottomright <- viewport(width = 0.5, height = 0.25, x = 0.97, y = 0.09, just = c("right", "bottom"))
...
library("grid")
## vp_usa <- viewport(width = 1, height = 0.6, x = 0.5, y = 0.5)
...
## png(filename = "map.png", width = 7, height = 4, units = "in", res = 300)
print(mainland)
print(alaska, vp = vp_alaska)
print(hawaii, vp = vp_hawaii)
## dev.off()

ggsave(file = "map.png")                # Just last viewport!


ggmain <- ggplot_build(mainland)
str(ggmain)
ggmain$layout$panel_scales_x[[1]]$range$range
ggmain$layout$panel_scales_y[[1]]$range$range
## c(-6381285, -5381285), ylim = c(-2116175, -1116175)


galaska <- ggplotGrob(ggplot(data = usa) +
    geom_sf(fill = "cornsilk") +
    coord_sf(crs = st_crs(3467), xlim = c(-2400000, 1500000), ylim = c(100000, 2500000), datum = NA))

ghawaii  <- ggplotGrob(ggplot(data = usa) +
    geom_sf(fill = "cornsilk") +
    coord_sf(crs = st_crs(4135), xlim = c(-161, -154), ylim = c(18, 23), datum = NA))
...
mainland +
    annotation_custom(
        grob = galaska,
        xmin = -2800000,
        xmax = -2800000+((1500000+2400000)/2.5),
        ymin = -2450000,
        ymax = -2450000+((2500000-100000)/2.5)
    ) +
    annotation_custom(
        grob = ghawaii,
        xmin = -1380000,
        xmax = -1380000+(-154+161)*120000,
        ymin = -2450000,
