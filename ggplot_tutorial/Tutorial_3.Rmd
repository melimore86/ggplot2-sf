...
## Tutorial #3
library("grid")
## library("cowplot")                      # Not loaded, as it changes ggplot theme
```
...
Example using two maps side by side, with a common legend in the
middle (three graph elements side by side).
...
```{r grid, fig.width = 15, fig.height = 5, out.width = "90%"}
levels(world$REGION)[7] <- "South America" # Simplification for the legend

gworld <- ggplot(data = world) +
    geom_sf(aes(fill = REGION)) +
    geom_rect(xmin = -100, xmax = -75, ymin = 12, ymax = 35, fill = NA, 
...
ggulf <- ggplot(data = world) +
    ...
    coord_sf(xlim = c(-100, -75), ylim = c(12, 35), expand = FALSE) +
    scale_fill_viridis_d(option = "plasma") +
...
Explain `plot_grid` from `cowplot`, and how this can be used to plot any ggplot objects together on a predefined grid (see https://cran.r-project.org/web/packages/cowplot/vignettes/plot_grid.html)

Note also the existence of `get_legend`, and that the legend can be used as any object.
...
## plot_grid(NorthAmerica, legend, Europe, nrow = 1, rel_widths = c(1, 
##     1.5, 1))
```
...
```{r legendmap_final}
## prow + draw_grob(legend, 2/4,0/8, 0.003) # No need for a grob.
```
