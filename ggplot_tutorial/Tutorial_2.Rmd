...
We can move the text slightly to be able to read better "South Carolina" and "Florida". For this, we create a new variable `nudge_y`, which is 0 for all states, 0.5 for Florida (moved slightly North), and -0.5 for South Carolina (slightly moved South):

```{r states-nudge}
states$nudge_y <- 0
states$nudge_y[states$ID == "Florida"] <- 0.5
states$nudge_y[states$ID == "South Carolina"] <- -0.5
```
...
flcities <- cbind(geocode(as.character(flcities$City), force = TRUE), flcities)
...
ggplot(data = world) +
    ...
    scale_fill_viridis(trans = "sqrt", alpha = .4) +
    coord_sf(xlim = c(-89.15, -76.12), ylim = c(24.65, 33.97), expand = FALSE) +
    scale_bar(lon = -88.66, lat = 25, distance_lon = 150, distance_lat = 25, 
        distance_legend = 50, dist_unit = "km", arrow_length = 75, 
        arrow_distance = 150, arrow_north_size = 6, orientation = TRUE, 
        legend_size = 3) +
